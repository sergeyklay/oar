...
Always use `describe.each` or `it.each` when testing multiple scenarios:

```typescript
// ❌ WRONG: Use describe.each or it.each for multiple scenarios
...
// ✅ CORRECT: Use describe.each for multiple scenarios
describe.each([
  { month: 3, name: 'April' },
  { month: 5, name: 'June' },
...
// ✅ CORRECT: Use it.each for multiple scenarios
it.each([
  { month: 3, name: 'April' },
  { month: 5, name: 'June' },
  { month: 8, name: 'September' },
  { month: 10, name: 'November' },
])('clamps 31st to 30th for 30-day month: $name', ({ month }) => {
  // ...
});
```
